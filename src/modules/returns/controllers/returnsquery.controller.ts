...
@ApiTags("Returns Query")
@UseGuards(ReturnsAuthGuard)
@ApiBearerAuth()
@ApiUnauthorizedResponse({ description: "Autenticación requerida." })
@Controller("returnss/query")
export class ReturnsQueryController {
  #logger = new Logger(ReturnsQueryController.name);

  constructor(private readonly service: ReturnsQueryService) {}

  @Get("list")
  @ApiOperation({ summary: "Get all returns with optional pagination" })
  @ApiResponse({ status: 200, type: ReturnssResponse })
  @ApiQuery({ name: "options", required: false, type: ReturnsDto }) // Ajustar según el tipo real
  ...
  async findAll(
    @Query("options") options?: FindManyOptions<Returns>    
  ): Promise<ReturnssResponse<Returns>> {
    try {
     
      const returnss = await this.service.findAll(options);
      logger.info("Retrieving all returns");
      return returnss;
    } catch (error) {
      logger.error(error);
  ...
  @ApiOperation({ summary: "Find returns by specific field" })
  @ApiQuery({ name: "value", required: true, description: 'Value to search for', type: String }) // Documenta el parámetro de consulta
  @ApiParam({ name: 'field', required: true, description: 'Field to filter returns', type: String }) // Documenta el parámetro de la ruta
  @ApiResponse({ status: 200, type: ReturnssResponse })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Returns no encontrados para la propiedad y valor especificado"
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("pagination")
  @ApiOperation({ summary: "Find returnss with pagination" })
  ...
  async findWithPagination(
    @Query() options: FindManyOptions<Returns>,
  ...
  @Get("count")
  @ApiOperation({ summary: "Count all returnss" })
  ...
  @Get("search")
  @ApiOperation({ summary: "Find and count returnss with conditions" })
  @ApiResponse({ status: 200, type: ReturnssResponse<Returns> })
  ...
  async findAndCount(
    ...
    try {
      ...
      if (!entities) {
        throw new NotFoundException(
          "Entidades Returnss no encontradas para el criterio especificado."
        );
      }
      return entities;
    } catch (error) {
  ...
  @Get("find-one")
  @ApiOperation({ summary: "Find one returns with conditions" })
  ...
  @ApiOperation({ summary: "Find one returns or return error" })
  @ApiResponse({ status: 200, type: ReturnsResponse<Returns> })
  @ApiQuery({ name: "where", required: true, type: Object }) // Ajustar según el tipo real
  @LogExecutionTime({
    layer: "controller",
  ...
  async findOneOrFail(
    ...
    try {
      ...
      if (!entity) {
        return new NotFoundException("Entidad Returns no encontrada.");
      }
      return entity;
    } catch (error) {
      logger.error(error);
  ...
  @Get(":id")
  @ApiOperation({ summary: "Get returns by ID" })
  @ApiResponse({ status: 200, type: ReturnsResponse<Returns> })
  @ApiResponse({ status: 404, description: "Returns not found" })
  @ApiParam({ name: 'id', required: true, description: 'ID of the returns to retrieve', type: String })
  @LogExecutionTime({
    layer: "controller",
    callback: async (logData, client) => {
      return await client.send(logData);
  ...
  async findById(@Param("id") id: string): Promise<ReturnsResponse<Returns>> {
    try {
      const returns = await this.service.findOne({ where: { id } });
      if (!returns) {
        throw new NotFoundException(
          "Returns no encontrado para el id solicitado"
        );
      }
      return returns;
    } catch (error) {
