...
import { logger } from '@core/logs/logger';
  ...
  export class ReturnsRepository {
    ...
    async findAll(options?: FindManyOptions<Returns>): Promise<Returns[]> {
      logger.info('Ready to findAll Returns on repository:',options);
    ...
    async findById(id: string): Promise<Returns | null> {
      const tmp: FindOptionsWhere<Returns> = { id } as FindOptionsWhere<Returns>;
      logger.info('Ready to findById Returns on repository:',tmp);
    ...
    async findByField(
      ...
      logger.info('Ready to findByField Returns on repository:',options);
    ...
    async count(): Promise<number> {
      logger.info('Ready to count Returns on repository...');
      let result= this.repository.count();
      logger.info('Was counted  instances of Returns on repository:');
      return result;
    }
    ...
    async findAndCount(where?: Record<string, any>): Promise<[Returns[], number]> {
      logger.info('Ready to findByField Returns on repository:',where);
    ...
    async findOne(options?: Record<string, any>): Promise<Returns | null> {
      if (!options || Object.keys(options).length === 0) {
        logger.warn('No conditions provided for finding Returns.');
        return null;
      }
      ...
      if (isFindOneOptions) {
        logger.info('Ready to findOne (FindOneOptions) Returns:', options);
        return this.repository.findOne(options as any);
      }
      const tmp: FindOptionsWhere<Returns> = options as FindOptionsWhere<Returns>;
      logger.info('Ready to findOneBy Returns on repository:', tmp);
      return this.repository.findOneBy(tmp);
    }
    ...
    async findOneOrFail(where?: Record<string, any>): Promise<Returns> {
      logger.info('Ready to findOneOrFail Returns on repository:',where);
    ...
    async update(
        ...
        logger.info('Updated instance of Returns with id:  was finded on repository:', instance);
